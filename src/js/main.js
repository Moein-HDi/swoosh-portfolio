...
class MagneticObject {
  ...
  const button = document.querySelector('#cv_btn');
...
//   dark and light theme
  $.getElementById('theme_btn').addEventListener('click', function () {
    if ($.body.classList.contains('light')) {
      this.firstElementChild.classList.replace('uil-moon', 'uil-sun');
    } else {
      this.firstElementChild.classList.replace('uil-sun', 'uil-moon');
    }
    $.body.classList[$.body.classList.contains('light') ? 'remove' : 'add'](
      'light'
    );
  });